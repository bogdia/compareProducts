...
def step_impl(context):
    ...
    log.debug ('Loaded a: ' +str( context.qa.quality))
...
@given(u'"product" with "aquality" and "bquality"')
def step_impl(context):
  ...
  for row in context.table:
    ...
    for head in context.table.headings[1:]:
      ...
      qavalue=int (context.qa.quality[aq])
      log.debug ("qavalue" +str( qavalue))
      score+=qavalue
    context.products.add_product(product, score)
    log.debug ("products are: " + str( context.products.list))
